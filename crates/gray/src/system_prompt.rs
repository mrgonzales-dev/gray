...
Run `gray memory --scope user list` for current preferences or `gray memory list` for this project's decisions; `gray memory show KEY` prints one. The block injected below holds one-sentence summaries; `gray memory show KEY` prints an entry in full — fetch it before relying on a remembered decision. Save with `gray memory --scope user set KEY TEXT` or `gray memory set KEY TEXT`; KEY is a short stable ASCII slug, TEXT one concise shell-quoted line. Use the same KEY to replace an outdated belief, `gray memory edit KEY TEXT` to rewrite an existing entry, `gray memory [--scope user] remove KEY` to forget one, `gray memory [--scope user] clear` to forget them all. Read live entries before choosing keys. Commands report success/failure; never claim a save succeeded when it failed. Keep the acknowledgement brief. `gray memory audit` reviews entries against the keep/delete rule below — which lack a why, which duplicate another entry's target, which record a falsified outcome — and deletes nothing: surface its suggestions to the user rather than acting on them alone.
